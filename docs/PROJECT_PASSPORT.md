...
Статус: офлайн-пайплайн и пять бизнес-маршрутов FastAPI реализованы; интерфейс Next.js разрабатывается отдельно. Полный датасет обработан локально, три CSV созданы и проверены. Живой вызов OpenAI ожидает серверный API-ключ; fallback проверен без ключа.
...
Это рабочая CLI-команда. Она запускает импорт, проверки, метрики, роли, кластеры, ранжирование и все выгрузки; при нарушении обязательной схемы завершается ненулевым кодом. Установка зависимостей и запуск API описаны в корневом README. На выданном наборе расчёт сохранил все 2 248 узлов и занял менее двух секунд после импорта зависимостей.
...
3. Сверить агрегаты transactions по src/dst с edges: n_tx — точно; суммы — с допуском `0.01 * n_tx` KZT на округление. При расхождении добавить предупреждение `PAIR_COUNT_MISMATCH` или `PAIR_AMOUNT_MISMATCH` в результат и продолжить расчёт. Edges остаётся источником сетевых метрик; transactions — общего оборота и временного графика. Предупреждение явно сообщает о расхождении источников.
