...
Переключатели в шапке доступны на главной, в рабочем пространстве, методологии и экспорте. Языки: қазақша (`kk`), русский (`ru`), English (`en`); темы: светлая и тёмная. Переводы интерфейса заранее подготовлены в `src/locales/{kk,ru,en}/{common,landing,workspace,extras}.json`. Сетевой сервис перевода не требуется. Для новой строки добавляйте одинаковый ключ и одинаковые параметры `{name}` во все три языка; `useI18n(namespace)` подставляет текст из каталога.

Выбор хранится год в cookies `money-graph-locale` и `money-graph-theme`. Сервер читает их при загрузке, поэтому HTML сразу получает правильные `lang` и `data-theme`; страницы рендерятся динамически. Неизвестные значения настроек дают русский язык и светлую тему. Полученный от API свободный текст доказательств и ответов сохраняется в исходном виде: контракт бэкенда пока не принимает язык. Демо-пояснения переведены вместе с интерфейсом.
